...
N = 100
# 生成网格函数
def generate_grid(n,p): # n: 网格大小 p: 概率
    return((np.random.rand(n,n) < p).astype(int))
# 判断连通性函数
def is_ocean(grid,n): # BFS搜索是否存在联通地图左右的水路
    visited = np.zeros_like(grid,dtype=bool) # 标记已访问的格子
    q = deque() # 存储待检查的格子
    ...
    while q: # BFS搜索
        ...
        for dx,dy in dirs:
            ...
            if 0 <= nx < n and 0 <= ny < n and not visited[nx,ny] and grid[nx,ny] == 1: # 搜索相邻的格子
                q.append((nx,ny))
                visited[nx,ny] = True
